...
// Dedup cache: prevents duplicate messages when agent sends via IPC and also returns the same text as output
const recentOutboundMessages = new Set<string>();
const DEDUP_TTL_MS = 30_000;

function trackOutbound(chatJid: string, text: string): void {
  const key = `${chatJid}:${text.slice(0, 500)}`;
  recentOutboundMessages.add(key);
  setTimeout(() => recentOutboundMessages.delete(key), DEDUP_TTL_MS);
}
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const output = await runAgent(group, prompt, chatJid, async (result) => {
    ...
    if (result.result) {
      ...
      if (text) {
        // Deduplicate: skip if this text was already sent via IPC send_message
        const dedupeKey = `${chatJid}:${text.slice(0, 500)}`;
        if (recentOutboundMessages.has(dedupeKey)) {
          logger.debug({ group: group.name }, 'Skipping duplicate output (already sent via IPC)');
        } else {
          trackOutbound(chatJid, text);
            ...
            try {
              ...
              if (data.type === 'message' && data.chatJid && data.text) {
                ...
                if (
                  ...
                  trackOutbound(data.chatJid, data.text);
                  await sendMessage(
                    data.chatJid,
