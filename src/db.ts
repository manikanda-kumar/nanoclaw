...
export function storeMessage(
  msg: NewMessage | proto.IWebMessageInfo,
  chatJid?: string,
  isFromMe?: boolean,
  pushName?: string,
): void {
  if (chatJid !== undefined && isFromMe !== undefined) {
    const waMsg = msg as proto.IWebMessageInfo;
    if (!waMsg.key) return;
